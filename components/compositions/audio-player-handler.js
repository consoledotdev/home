export default class AudioPlayerHandler {
    constructor(p) {
        ...
        this.bind();
        this.exportToWindow(p);
    }

    exportToWindow(p) {
        if (!window.CNSL?.players || window.CNSL.players.length == 0) {
            window.CNSL = {
                players: [this],
            };
        } else {
            let exists = window.CNSL.players.find((p) => p.public.getId() == this.id);
            if (exists) this.unbind();
            else window.CNSL.players.push(this);
        }
    }

    remove() {
        window.CNSL.players = window.CNSL.players.filter((p) => {
            return p.public.getId() != this.id;
        });
        this.unbind();
    }
    ...
    unbind() {
        this.progress.removeEventListener("mousedown", this.dragTime);
        this.progress.removeEventListener("mousemove", this.dragTime);
        this.progress.removeEventListener("mouseup", this.stopDragging);
        this.progress.removeEventListener("mouseout", this.stopDragging);
        this.playBtn.removeEventListener("click", this.play);
        this.pauseBtn.removeEventListener("click", this.pause);
        this.titleWrapper.removeEventListener("mouseover", this.hoverScrollTitle);
        this.titleWrapper.removeEventListener("mouseout", this.hoverScrollTitleStop);
        if (this.titleAlt) this.titleWrapper.removeEventListener("mouseover", this.hoverScrollTitleAlt);
        if (this.titleAlt) this.titleWrapper.removeEventListener("mouseout", this.hoverScrollTitleAltStop);
    }
    ...
    async play() {
        try {
            ...
            this.wrapper.classList.remove("is-at-start");
            this.scrollTitle();
    ...
    pause() {
        ...
        if (this.timeElapsed.textContent == "00:00") this.wrapper.classList.add("is-at-start");
    ...
    checkOtherPlayers() {
        window.CNSL.players.forEach((p) => {
            if (p.public.getId() != this.id && p.public.isPlaying()) p.public.stopPlaying();
        });
    }
